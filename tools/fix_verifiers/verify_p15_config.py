...
import logging, sys, os, shutil, tempfile
...
chk("DropForward=150 / penalty=10", C.Placement.DROP_FORWARD_MM == 150.0 and C.Thresholds.PLACEMENT_PENALTY_PER_TARGET == 10)
...
data = data.replace("drop_forward_mm: 150", "drop_forward_mm: 220")
open(p,"w").write(data)
...
chk("drop_forward 150 → 220", C.Placement.DROP_FORWARD_MM == 220.0)
...
chk("drop_forward 回到 150", C.Placement.DROP_FORWARD_MM == 150.0)
shutil.rmtree(tmp)
