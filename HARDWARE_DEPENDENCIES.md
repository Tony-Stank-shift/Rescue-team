...
底盘层（电机/编码器/里程计）由另一位同学负责，**接口必须先约定**：

- 上层 → 底层：速度指令 `(v, w)`（v=线速度 mm/s，w=角速度 rad/s）。
- 底层 → 上层：位姿 `(x, y, theta)`（x/y=场地坐标 mm，theta=朝向 rad，0=+Y 车头方向）。

**待和底盘同学确认**：给的是「位姿」还是「轮速/里程」、速度单位、坐标系定义（朝向 0 指向）。
