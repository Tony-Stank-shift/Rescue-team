...
（`compileall` / `run_all.py` / `hw_selftest --mock` PASS=7 FAIL=0 SKIP=7 / 决策引擎自测 exit 0）+ **集成仿真 5 种子**。
`run_all.py` 的套数**随新增验证器增长**（结项批 7 套 → 2026-09-17 新增搜索覆盖后 **8 套**），
引用时请写"实测当时的套数"而不是固定数字。
...
套取方式、单趟容量（1）**均未变**；舵机行程**已于 2026-09-18 由 0°/70° 现场标定为
**0°/85°**（脉宽 1000us / 1944us，对齐固件 `servo.h` 的 `SERVO_RAISED_ANGLE_DEG`）。
⚠️ 本文档与 `CODE_AUDIT.md` / `COMPLIANCE_AUDIT*.md` 里凡是写「0°~70°」的地方，
都是**标定之前的快照描述**，以本条与 `servo.h` 为准。
...
| **V2-3** | **推升来源变了，`place_ramp()` 的语义已失效** | 中 | 旧机构靠舵机带**单块**后方板渐进上调；V2 是三块**固定**阶梯板。旧代码"推入中 0°→70° 分 4 步渐进抬"（现为 0°→85°）的**原有升力作用已消失**，只剩渐进卸力。已把 `_place_steps` 提为 YAML 可调 `placement.progressive_raise_steps`（4=旧行为 / 0=保持套住到到位后一次释放），属真机标定项。 |
...
# 追加：搜索算法优化（2026-09-17，依据赛规图7）

**触发**：赛规第 16 页的现场布置图（图7）显示 20 个目标**紧凑堆在场地正中**
（像素级量得 6×4 格 ≈ **240×160mm**，物体紧挨着），而原搜索算法没按这个前提设计。

## 原算法的问题（`decision_engine._get_explore_target`）

| # | 问题 | 后果 |
|---|---|---|
| 1 | 纯随机、**无记忆** | 反复走同一片，无覆盖保证 |
| 2 | 随机范围实际只有 `[900,2100]²`（占全场 16%） | **外圈 900mm 环带永远搜不到** |
| 3 | `clamp(200,2800)` **永远不生效** | 注释声称"修过 y>2200 搜不到"，**真凶还在** |
| 4 | 兜底同样被限制在 `[900,2100]²` | 外圈连兜底都到不了 |

## 改成

**确定性搜索计划**：**中央优先**（停在距场心 600mm 处观察，不压上物体堆）
+ **全场蛇形 4 条车道**（y=550/1000/2000/2450）+ **全场地随机兜底**。

关键约束（推导见 `docs/SEARCH_STRATEGY.md`）：

- **不能直奔场心**：按图7 那是一坨紧挨的物体，开过去就是碾过去 →
  车道距场心 ≥500mm，**整条路径距场心最近正好 500mm**；
- 车道单侧可靠探测半宽 **理论 708mm**（FOV 圆锥 + 200px² 面积门限联立），工程取 **500mm**；
- 车道避开顶/底安全区（进对方安全区直接判本轮结束）。

## 验证与**保真度缺口**

新增 `tools/fix_verifiers/verify_search_coverage.py`（已并入 `run_all.py`，**15/15**）：
用几何可见性模型把 21 个路点离散成 98 个位姿，断言"作业区每 100mm 网格点都能被看到"、
"路径不穿越场心禁入区"、图7 中央堆首点即可见等。

> 🔴 **必须记住**：`snapshot_sim.py` 的仿真基线**不覆盖探索路径** ——
> 实测整场 9000 帧里 `_get_explore_target()` 调用 **0 次**、"无目标可用"只有 **7 帧**
> （原因：仿真把目标散布在半径 600~1414mm 环带里，相机总有目标可看）。
> **所以本改动"仿真基线没变"不构成任何验证**，必须引用上面那个专用验证器。
>
> 真实场景恰好相反：从 4 号区看场心中央堆距离 **1909mm**，远超 1138mm 探测上限
> → **开局必须探索**。即探索路径在仿真里被绕过、在真实场上却是关键路径。

## 已知残余

- **盲区** `x∈[800,900] × y∈[50,150]`（约 100×100mm，紧贴底边围栏）。
  车体 300mm 宽本来就没法贴到离围栏 150mm 内作业；随机兜底仍有机会覆盖。
  如需消除：`EXPLORE_LANE_EDGE_MM` 550→500、`EXPLORE_X_MARGIN_MM` 300→250（贴墙余量降到 100mm）。
- **未确认**：现场开赛时目标到底集中在正中还是散布全场（**赛规自相矛盾**：
  图7 画的是集中，同页文字说"位置可随机"，且"如图6 所示"**串到了搬运赛道的图号**——
  全文图1~图6 全属智能搬运赛道）。两种都已兜住，确认后可进一步优化
  （见 `docs/SEARCH_STRATEGY.md` §9）。

## 回归结论（本轮）

`compileall` OK、`run_all` **8/8**（新增 SEARCH 项 15/15）、
`hw_selftest --mock` PASS=7 FAIL=0 SKIP=7、决策引擎自测 exit 0、
单元测试 27/27、`snapshot_sim.py` → **80/80/80/85/80** 逐位一致。

> 注意最后一项对本改动**无证明力**（原因见上）；它只说明非探索路径没被弄坏。

---

# 追加：套取链修复（2026-09-17 真机 · 4 号区）

现场症状（日志 `09:12:28–09:14:49`）：
① 「识别到物体却不去套，反而移动一段距离，之后物体就不在视野里了」
② 「物体堆卡在车下」
③ 「一进 AUTONOMOUS 就报 `NO_ACTION_15S → EMERGENCY_STOP`」

**完整分析与证据见 [`PICKUP_CHAIN_FIX_20260917.md`](PICKUP_CHAIN_FIX_20260917.md)**，摘要：

| 缺陷 | 后果 |
|---|---|
| `motion_control` 角度误差 >25.8° 时 `linear` 被压成 0 | 只原地转 → 位置不变 → 看门狗判"卡死" |
| 近距仍要求"先对准" | 方位角抖动 >25.8° ⇒ **最后 10cm 永远进不去**（目标 `(1527,1522)` 挂了 87 秒没进去） |
| 看门狗只按**位置**判"在动" | 合法转向被误判卡死 → 10s 后 `explore()` 抢走导航目标 → 物体出视野 |
| 到位判在**车心**（开口中心在车心前方 70mm） | 车心压上物体才算到位 ⇒ **整车压过物体堆** |
| `start_match()` 未复位空闲计时器 | DEBUG 待机 + 1.5m 开场退避被算成"无动作" ⇒ 开局误报急停 |

**另查出一处"文档声称已实现、代码里根本没有"**：`SLEEVE_OPENING_MM`
（矩形开口判据）**从未被任何逻辑引用** —— 配置与注释写着"捕获判据必须用它、
现已按矩形开口复核"，但全仓库只有圆形 `CAPTURE_RADIUS_MM`。本次补上实现。

回归：`run_all` **9/9**（新增 PICKUP 32/32）、`hw_selftest` PASS=7 FAIL=0 SKIP=7、
单元测试 27/27、**规范仿真 85/80/80/85/80**（原基线 80/80/80/85/80，seed 1 +5 分）。

> ⚠️ **留档一条弯路**：修"压到车下"时第一版是把**导航目标往前挪 70mm**，
> 得到的是**每帧随车重算的移动设定点** → 车追一个后退的目标、永远到不了，
> **仿真 5 个种子直接掉到 0 分**。正确做法是导航目标保持物体本身、
> 把"到位"判据换到套取框。**是仿真基线抓住的** —— 详见新文档 §4。
